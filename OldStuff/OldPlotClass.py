...
from SocketMyData import d, plotMethod, desimaler
...
# If we have installed mplcursor, use it
Interactivity = True
try:
	import mplcursors
except:
...
class PlotObject:
	def __init__(self, nrows, ncols, queue, sharex=True):
		...
		# legger til hidden points så vi kan velge bare disse med mplcursors 
		self.hiddenPoints = []
	...
	def stopPlot(self):
		...
		for lineInfo in self.lines.values():
			...
			if dif > 0 :
				...
				if Interactivity:
					hiddenpoints = subplot.scatter(self.Data[xListName][:-dif],self.Data[yListName], color='none')
			else:
				subplot.plot(
					self.Data[xListName], 
					self.Data[yListName], 
				...
				if Interactivity:
					hiddenpoints = subplot.scatter(self.Data[xListName],self.Data[yListName], color='none')
			...
			if Interactivity:
				self.hiddenPoints.append(hiddenpoints)

		if Interactivity:
			mplcursors.cursor(self.hiddenPoints, hover=2)
			mplcursors.cursor(self.hiddenPoints, multiple=True)
		plt.show()
