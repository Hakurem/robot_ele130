...
# Setter opp søkestier og importerer pakker
import os
import sys
...
Configs.plotMethod = 0		# (1,2,3) mulige metoder å plotte på (hver med sine fordeler og ulemper).
...
d.Ts = []             # tidsskritt
d.EulerFListe = []   # f.eks kan beregninger av euler forward legges inn her (kan gi et annet navn om du vil)
"""
...
def setPorts(r, devices, port):
	r.ColorSensor = devices.ColorSensor(port.SX) # lyssensoren
	"""
...
def addMeasurements(d,r,_G,k):
	if k==0:        
		...
		_G.start_lysverdi = r.ColorSensor.reflection() 	# lagrer første lysmåling som kan brukes til å beregne Flow
		d.Tid.append(0)
	else: 
...
def MathCalculations(d,_G,k):
    ...
    if k == 0:
        #d.Flow.append(...)  <-- tips: hva er (første lysmåling minus første lysmåling)
        #d.EulerFListe.append(...) <-- hva skal legges inn her?
        pass
    else:
        ...
        # Beregn Flow
        #d.Flow.append(...) <-- tips: første lysmåling er her inni denne variabelen "_G.start_lysverdi"
...
def lagPlot():
	...
	# TODO: Legg til subplots
	nrows = 2
	ncols = 2
	sharex = False

	#++++++++Lager objekt fra plottklassen og sender nødvendig data++++
	plt = Plotter(nrows = nrows, ncols = ncols, sharex = sharex)
	ax = plt.ax
	#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
# Definer hvilke variabler du vil plotte i online eller offline
def plotData(plt):
	ax = plt.ax
